...
function svg2vectorDrawableContent(svgContent) {
    return new Promise(function (resolve, reject) {
        const svg = xml.parseString(svgContent);
        const style = getStyle(svg.childs);
        ...
        // XML code
        let vectorDrawableXML = '<?xml version="1.0" encoding="utf-8"?>\n\
<vector xmlns:android="http://schemas.android.com/apk/res/android"\n\
        ...
        function travel(obj, indent, fill = '#000000') {
            if(!obj){
                return;
            }
            ...
            try {
                for (let i = 0; i < obj.length; i++) {
                    ...
                    if (obj[i].name == 'g') {
                        ...
                        travel(obj[i].childs, indent, fill);
                        vectorDrawableXML += repeatString(' ', indent) + '</group>\n';
                        ...
                        if (/(rect)/i.test(obj[i].name)) {
                            let x = obj[i].attrib.x ? parseFloat(obj[i].attrib.x) : 0;
                            let y = obj[i].attrib.y ? parseFloat(obj[i].attrib.y) : 0;
                            let width = parseFloat(obj[i].attrib.width);
                            let height = parseFloat(obj[i].attrib.height);
                            let rx = obj[i].attrib.rx ? parseFloat(obj[i].attrib.rx) : 0;
                            let ry = obj[i].attrib.ry ? parseFloat(obj[i].attrib.ry) : 0;
                            ...
                            let cx = obj[i].attrib.cx ? parseFloat(obj[i].attrib.cx) : 0;
                            let cy = obj[i].attrib.cy ? parseFloat(obj[i].attrib.cy) : 0;
                            let r = parseFloat(obj[i].attrib.r);
...
function formatColor(hexColor) {
    ...
    if (/rgb\(\d+,\d+,\d+\)/i.test(hexColor.replace(/\s*/g, ''))) {
        const rgb = hexColor.replace(/rgb\(/i, '').replace(/\)/, '').split(',');
        hexColor = '#' + numberToHex(rgb[0]) + numberToHex(rgb[1]) + numberToHex(rgb[2]);
        return hexColor.toUpperCase();
    }
    if (/rgb\(\d+%,\d+%,\d+%\)/i.test(hexColor.replace(/\s*/g, ''))) {
        const rgbPercent = hexColor.replace(/rgb\(/i, '').replace(/\)/, '').replace(/%/g, '').split(',');
...
function ellipseToPath(cx, cy, rx, ry) {
    const controlDistanceX = rx * 0.5522847498307935;
    const controlDistanceY = ry * 0.5522847498307935;
